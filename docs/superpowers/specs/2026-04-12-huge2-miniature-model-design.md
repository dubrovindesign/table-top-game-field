...
- Default distance unit rules treat `huge2` like hexon-based classes (`big`, `huge`).
...
- Ensure roster size sorting rank includes `huge2` in a stable place between `huge` and neighboring classes according to UI policy ("small -> huge").
...
2. Move each matching entry into `huge2` collections preserving:
...
3. Remove migrated entries from `huge` arrays.
4. Keep migration idempotent (reloading an already-migrated state makes no additional changes).
...
- If migration finds malformed card/runtime pair, keep item in original `huge` collection and log a warning (do not drop data).
...
- Multiplayer drag/state sync still works with mixed sizes.
